...
// Convert escaped newlines to real newlines
  ...
  const backendUrl = props.getProperty('BACKEND_URL') || 'https://example.com';
  ...
  const payload = {
    ...
    iat: now,
    exp: now + 3600,
    target_audience: backendUrl  // Must match backend AUDIENCE so it can validate the id_token
  };
